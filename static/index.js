...
function processMonsters(raw_data) {
    ...
    for (let line of data) {
        ...
        if (line[3].includes("Wiki: ")) {
            wiki_link = line[3].substr(line[3].indexOf("Wiki: ") + 7, line[3].indexOf("\"", line[3].indexOf("Wiki: ") + 7) - line[3].indexOf("Wiki: ") - 7);
        }
...
$.get("https://raw.githubusercontent.com/kolmafia/kolmafia/main/src/data/monsters.txt", processMonsters);
...
function initialSetup() {
    ...
    for (let element of document.getElementsByClassName("setting")) {
        result.settings[element.getAttribute("setting")] = (getCookie("EGGNET_setting_" + element.getAttribute("setting"), "0") != "0");
    }

    // Set 0 eggs for copyable monsters
    for (let monster_id in result.monsters_by_id) {
        if (result.monsters_by_id[monster_id].copyable && !(monster_id in result.eggs)) {
    ...
    document.getElementById("last_update_ts").textContent = last_update_date.toGMTString();
    ...
    document.getElementById("total_progress").getElementsByClassName("barfill")[0].style.setProperty("--percentage", (100 * collected_eggs / total_eggs) + "%");
    document.getElementById("total_progress").getElementsByClassName("eggs-total")[0].textContent = collected_eggs + "/" + total_eggs + " eggs donated (" + (Math.round(10000 * collected_eggs / total_eggs) / 100) + "%)";
    ...
    for (let monster_id in result.eggs) {
        ...
        name.innerHTML = "<a href='" + WIKI_WEBPAGE + "/" + monster_data.wiki + "' target='_blank'>" + monster_data.name + "</a>" + (result.eggs[monster_id] == 100 ? "" : (" (" + result.eggs[monster_id] + "/100 eggs)"));
    ...
    for (let element of document.getElementsByClassName("setting")) {
        if (result.settings[element.getAttribute("setting")]) element.checked = true;
    }
}
...
function refreshMonsterList() {
    ...
    result.monster_list.sort(function(x, y) {
        ...
        if (result.settings.current_tab == "completion") {
            if (result.eggs[x.id] != result.eggs[y.id]) return -cmp(result.eggs[x.id], result.eggs[y.id]);
            else return cmp(x.id, y.id);
        }
        if (result.settings.current_tab == "ascension") {
            if (result.scotch_list_ranking[x.badge ? x.badge : "default"] != result.scotch_list_ranking[y.badge ? y.badge : "default"]) return cmp(result.scotch_list_ranking[x.badge ? x.badge : "default"], result.scotch_list_ranking[y.badge ? y.badge : "default"]);
            else return cmp(x.id, y.id);
        }
    });

    for (let monster of result.monster_list) {
        if (result.settings.hide_completed && result.eggs[monster.id] == 100) continue;
